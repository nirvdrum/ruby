...
#if USE_ZJIT
// Write a ZJIT frame's off-stack Ruby slots into VM stack memory using its
// stack map, decoding downward from cfp->sp. Does not touch frame metadata.
static void
zjit_write_frame_stack(const rb_control_frame_t *cfp, const zjit_jit_frame_t *jit_frame)
{
...
// Synthesize control frames for the virtual inline frames a chain-bearing
// JITFrame describes. `cfp` is the physical frame; its JITFrame's chain has
// one entry per logical frame, innermost first, with the final entry
// describing the physical frame itself. On return, the synthesized frames
// occupy the slots directly below `cfp` (younger frames, if any, have been
// shifted down to make room) and ec->cfp points at the youngest frame.
//
// Layout reconstruction needs no dedicated metadata: the stack map's shape
// mirrors the frame layout. Each logical frame contributes its operand
// stack entries, then a Skip covering its env data and local table (locals
// are eagerly spilled and already in memory; env slots are written here
// from the descriptors), then one entry for its receiver.
static void
zjit_synthesize_inline_frames(const rb_execution_context_t *ec_const, rb_control_frame_t *cfp, const zjit_jit_frame_t *jit_frame, bool doomed)
{
    // Materialization conceptually mutates execution context state (ec->cfp
    // moves to the youngest synthesized frame); the const on the parameter
    // chain reflects the common read-mostly case, not this one.
    rb_execution_context_t *ec = (rb_execution_context_t *)ec_const;

    uint32_t virtual_count = jit_frame->inline_count - 1;
    VM_ASSERT(virtual_count > 0);

    // Every synthesized frame retires the materialization debt its inline
    // push recorded. Settle it up front so a collapse below settles too.
    ec->zjit_inline_frame_debt -= virtual_count * sizeof(rb_control_frame_t);

    // Make room directly below the physical frame by shifting any younger
    // frames (e.g. cfunc frames pushed from inlined code) down. Interior
    // pointers into the shifted frames are rebased below; ec->tag->cfp only
    // references frames older than `cfp` and needs no adjustment.
    rb_control_frame_t *youngest = ec->cfp;
    VM_ASSERT(youngest <= cfp);
    size_t younger_bytes = (char *)cfp - (char *)youngest;
    rb_control_frame_t *new_youngest = youngest - virtual_count;

    // JIT stack overflow checks reserve this room through the debt counter,
    // but interpreter frames pushed during fallback calls do not know about
    // the debt and can consume it. That can only strand a chain whose native
    // continuation is being discarded by a longjmp, in which case the
    // virtual frames can be collapsed instead of synthesized: no inlined
    // callee can own a catch table entry (ISEQs with exception handlers are
    // never compiled, so never inlined), so the exception machinery only
    // needs the physical frame's own location. The frames' values are dead,
    // so the stack is reported empty to keep GC away from stale slots.
    if ((VALUE *)new_youngest <= cfp->sp) {
        VM_ASSERT(doomed);
        const zjit_inline_frame_t *physical = &jit_frame->inline_frames[virtual_count];
        cfp->pc = physical->pc;
        cfp->_iseq = (rb_iseq_t *)physical->iseq;
    ...
    if (younger_bytes > 0) {
        memmove(new_youngest, youngest, younger_bytes);

        // Iseq and ifunc block handlers are tagged pointers to a captured
        // block that lives inside the block-supplying frame's control frame
        // (&cfp->self). Rebase any handler that points into the region that
        // just moved. Handlers are stored in environment specval slots,
        // which live on the VM value stack and did not move, and in
        // ec->passed_block_handler. The other tagged specval form, a
        // guarded previous EP, points into the value stack or a heap
        // environment and can never fall inside the control frame region,
        // so the range check below cannot misfire on it.
        ptrdiff_t shift_bytes = (char *)youngest - (char *)new_youngest;
        rb_control_frame_t *moved_end = new_youngest + (younger_bytes / sizeof(rb_control_frame_t));
        for (rb_control_frame_t *moved = new_youngest; moved < moved_end; moved++) {
            VALUE *specval_slot = (VALUE *)moved->ep - 1;
            VALUE specval = *specval_slot;
            if ((specval & 0x03) != 0 && (char *)specval >= (char *)youngest && (char *)specval < (char *)cfp) {
                *specval_slot = specval - shift_bytes;
            }
        }
        VALUE passed = ec->passed_block_handler;
        if ((passed & 0x03) != 0 && (char *)passed >= (char *)youngest && (char *)passed < (char *)cfp) {
            ((rb_execution_context_t *)ec)->passed_block_handler = passed - shift_bytes;
        }

        // A throw in flight during the shift holds the same class of interior
        // pointer: the THROW_DATA in ec->errinfo records the catch frame the
        // unwind resumes at, and a break or return target can be younger than
        // the chain frame being synthesized (a longjmp discards native
        // continuations, but vm_exec resumes VM frames above the tag, so
        // shifted frames are not necessarily dead). Rebase it so the handler's
        // frame search does not match a synthesized frame at the stale address.
        VALUE errinfo = ec->errinfo;
        if (!SPECIAL_CONST_P(errinfo) && THROW_DATA_P(errinfo)) {
            const rb_control_frame_t *catch_frame = THROW_DATA_CATCH_FRAME((struct vm_throw_data *)errinfo);
            if ((char *)catch_frame >= (char *)youngest && (char *)catch_frame < (char *)cfp) {
                THROW_DATA_CATCH_FRAME_SET((struct vm_throw_data *)errinfo,
                                           (const rb_control_frame_t *)((char *)catch_frame - shift_bytes));
            }
        }
    }
    ec->cfp = new_youngest;

    // Fill operand stacks and receiver slots for every logical frame from
    // the stack map, when one is attached. Sites that spill the stack
    // eagerly (fallback and ccall preparation) attach no map; their values
    // are already in VM stack memory. cfp->jit_return must still be valid
    // here for VREG decoding.
    zjit_write_frame_stack(cfp, jit_frame);

    // Lay out the synthesized frames from the descriptors. ZJIT's SP
    // register is pinned at the physical frame's initial stack pointer, and
    // each descriptor records its frame's slot offset from it. The base is
    // recovered from cfp->sp minus the JITFrame's recorded save offset;
    // every site that publishes a JITFrame saves cfp->sp in the same
    // instruction sequence, so the pair is always consistent. It cannot be
    // derived from cfp->ep because the environment can escape to the heap
    // (Binding or block capture) while the frame is live, leaving cfp->ep
    // pointing outside the VM stack. A frame's env flags sit one slot below
    // its stack base, and its region ([receiver][locals][env data]) sits
    // below that. An outer frame is suspended at its call into the frame
    // above, so its stack top is the inner frame's receiver slot (receiver
    // and arguments conceptually consumed by the call).
    VALUE *sp_reg = cfp->sp - jit_frame->sp_offset;
    VALUE *inner_recv_slot = NULL;

    for (uint32_t level = 0; level < virtual_count; level++) {
        const zjit_inline_frame_t *vframe = &jit_frame->inline_frames[level];
        size_t gap = 1 + ISEQ_BODY(vframe->iseq)->local_table_size + VM_ENV_DATA_SIZE;

        VALUE *ep = sp_reg + vframe->sp_base - 1;
        VALUE *recv_slot = sp_reg + vframe->sp_base - gap;
        VALUE *stack_top = level == 0 ? cfp->sp : inner_recv_slot;
        VM_ASSERT(stack_top >= sp_reg + vframe->sp_base);

        // Write the env slots from the descriptor.
        ep[VM_ENV_DATA_INDEX_ME_CREF] = (VALUE)vframe->cme;
        ep[VM_ENV_DATA_INDEX_SPECVAL] = vframe->specval;
        ep[VM_ENV_DATA_INDEX_FLAGS] = vframe->frame_type;

        // Synthesize the control frame. Level 0 is the innermost (youngest)
        // logical frame and lands furthest from the physical frame.
        rb_control_frame_t *vcfp = cfp - virtual_count + level;
        vcfp->pc = vframe->pc;
        vcfp->sp = stack_top;
        vcfp->_iseq = (rb_iseq_t *)vframe->iseq;
        vcfp->self = *recv_slot;
        vcfp->ep = ep;
        vcfp->block_code = NULL;
        vcfp->jit_return = 0;

        inner_recv_slot = recv_slot;
    }

    // The final chain entry describes the physical frame: restore its own
    // pc, iseq, and stack top (its sp field held the global top so the map
    // decode above could start there). Its stack top is the outermost
    // virtual frame's receiver slot, like any other suspended caller.
    const zjit_inline_frame_t *physical = &jit_frame->inline_frames[virtual_count];
    cfp->pc = physical->pc;
    cfp->_iseq = (rb_iseq_t *)physical->iseq;
    cfp->sp = inner_recv_slot;
    cfp->jit_return = 0;
}

// Materialize JITFrame-enabled CFP into interpreter-compatible CFP.
// longjmp_path is true when called during stack unwinding, in which case
// every frame above the target is being discarded.
static void
zjit_materialize_frames(const rb_execution_context_t *ec, rb_control_frame_t *cfp, bool materialize_target, bool longjmp_path)
{
    ...
    while (true) {
        ...
        if (CFP_ZJIT_FRAME_P(cfp)) {
            ...
            // A chain-bearing JITFrame represents several logical frames;
            // synthesize control frames for the virtual ones instead of the
            // single-frame fill-in below. Frames above the target during a
            // longjmp are being discarded and may be collapsed if the
            // synthesis room was consumed by interpreter pushes.
            if (jit_frame->inline_count > 0) {
                bool doomed = longjmp_path && cfp != end_cfp;
                zjit_synthesize_inline_frames(ec, cfp, jit_frame, doomed);
...
{
    zjit_materialize_frames(ec, cfp, true, false);
}
...
rb_zjit_materialize_frames_for_longjmp(const rb_execution_context_t *ec, rb_control_frame_t *cfp)
{
    ...
    zjit_materialize_frames(ec, cfp, !ec->tag->zjit_frame_active, true);
}
#endif
